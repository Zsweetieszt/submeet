...
<body>
    <div class="sidebar sidebar-dark sidebar-fixed border-end" id="sidebar">
        ...
        {{-- {{dd(Auth::user()->user_events->load('event')->pluck('event.event_code')->toArray());}} --}}
        <?php
...
$hasAccessNonPresenter = Auth::user()
    ...
    ->user_events()
    ->whereHas('role', function ($q) {
        $q->whereIn('role_name', ['Non-Presenter', 'Presenter']);
    })
    ->whereHas('event', function ($q) {
        $q->where('event_code', request()->route('event'));
    })
    ->first()->is_offline ?? null;

        ?>

        <ul class="sidebar-nav" data-coreui="navigation" data-simplebar>
            <li class="nav-item"><a class="nav-link" href="{{ route('dashboard.event', request()->route('event')) }}">
                    <i class="cil-cursor nav-icon"></i> Current Event</a></li>

            @if ($hasAccessPresenter || Auth::user()->root)
                <li class="nav-item"><a class="nav-link" href="{{ route('index.paper', request()->route('event')) }}">
                        <i class="cil-file nav-icon"></i> My Papers </a></li>
            @endif

            @if ($hasAccessPresenter || Auth::user()->root)
                <li class="nav-item"><a class="nav-link"
                        href="{{ route('index.camera-ready', request()->route('event')) }}">
                        <i class="cil-camera nav-icon"></i> Camera-ready Paper </a></li>
            @endif

            @if ($hasAccessPresenter || Auth::user()->root)
                <li class="nav-item"><a class="nav-link"
                        href="{{ route('index.supporting-materials', request()->route('event')) }}">
                        <i class="cil-cloud-upload nav-icon"></i> Supporting Materials </a></li>
            @endif

            @if ($hasAccessPresenter || ($hasAccessNonPresenter && $is_offline == true))
                <li class="nav-item"><a class="nav-link" href="{{ route('index.my-payments', request()->route('event')) }}">
                        <i class="cil-wallet nav-icon"></i> My Payments </a></li>
            @endif

            @if ($hasAccessReviewer || Auth::user()->root)
                <li class="nav-item nav-group">
                    <a class="nav-link nav-group-toggle" href="#">
                        <i class="cil-pen-nib nav-icon"></i> Reviewer
                    </a>
                    <ul class="nav-group-items">
                        <li class="nav-item">
                            <a class="nav-link" href="{{ route('events.reviewer', request()->route('event')) }}">
                                <span class="nav-icon"><span class="nav-icon-bullet"></span></span> Review Paper
                            </a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link" href="{{ route('events.reviewer.history', request()->route('event')) }}">
                                <span class="nav-icon"><span class="nav-icon-bullet"></span></span> Review History
                            </a>
                        </li>
                    </ul>
                </li>
            ...
            @if ($hasAccessEditor || Auth::user()->root)
                <li class="nav-item nav-group"><a class="nav-link nav-group-toggle" href="#">
                        <i class="cil-pencil nav-icon"></i> Editor </a>
                    <ul class="nav-group-items">
                        <li class="nav-item">
                            <a class="nav-link" href="{{ route('index.desk.evaluation', request()->route('event')) }}">
                                <span class="nav-icon"><span class="nav-icon-bullet"></span></span> Desk Evaluation
                            </a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link" href="{{ route('index.assign.reviewer', request()->route('event')) }}">
                                <span class="nav-icon"><span class="nav-icon-bullet"></span></span> Assign Reviewer
                            </a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link" href="{{ route('index.editor.decision', request()->route('event')) }}">
                                <span class="nav-icon"><span class="nav-icon-bullet"></span></span> Editor Decision
                            </a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link" href="{{ route('index.revision.paper', request()->route('event')) }}">
                                <span class="nav-icon"><span class="nav-icon-bullet"></span></span> Revision Paper
                            </a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link" href="{{ route('index.final.paper', request()->route('event')) }}">
                                <span class="nav-icon"><span class="nav-icon-bullet"></span></span> Final Paper
                            </a>
                        </li>
                    </ul>
                </li>
            ...
            @if ($hasAccessOrganizer || Auth::user()->root)
                <li class="nav-item nav-group"><a class="nav-link nav-group-toggle" href="#">
                        <i class="cil-briefcase nav-icon"></i> Organizer </a>
                    <ul class="nav-group-items">
                        <li class="nav-item"><a class="nav-link"
                                href="{{ route('organizer.event', request()->route('event')) }}">
                                <span class="nav-icon"><span class="nav-icon-bullet"></span></span> Event Settings</a>
                        </li>
                        <li class="nav-item"><a class="nav-link"
                                href="{{ route('organizer.users', request()->route('event')) }}">
                                <span class="nav-icon"><span class="nav-icon-bullet"></span></span> Participants</a>
                        </li>
                        <li class="nav-item"><a class="nav-link"
                                href="{{ route('organizer.payment-settings', request()->route('event')) }}">
                                <span class="nav-icon"><span class="nav-icon-bullet"></span></span> Payment Settings </a>
                        </li>
                        <li class="nav-item"><a class="nav-link"
                                href="{{ route('organizer.payment', request()->route('event')) }}">
                                <span class="nav-icon"><span class="nav-icon-bullet"></span></span> Payments </a>
                        </li>
                    </ul>
                </li>
            @endif
            ...
            <li class="nav-item"><a class="nav-link" href="{{ route('dashboard') }}">
                    <i class="cil-arrow-thick-to-left nav-icon"></i> Back to Dashboard </a></li>
        </ul>
    ...
    <div class="wrapper d-flex flex-column min-vh-100">
                ...
                <ul class="header-nav d-flex align-items-center">
                    ...
                    <li class="nav-item dropdown">
                        <button class="btn btn-link nav-link py-2 px-2 d-flex align-items-center" type="button"
                            ...
                            {{ Auth::user()->user_events->pluck('event')->where('event_code', request()->route('event'))->first()?->event_name ?? '' }}
                        </button>
                        <ul class="dropdown-menu dropdown-menu-end" style="--cui-dropdown-min-width: 8rem;">
                            @foreach (Auth::user()->user_events->pluck('event') as $event)
                                <li>
                                    <a class="dropdown-item d-flex align-items-center"
                                        href="{{ route('dashboard.event', $event->event_code) }}">
                                        {{ $event->event_name }}
                                    </a>
                                </li>
                            @endforeach
                    ...
                    <li class="nav-item dropdown"><a class="nav-link py-0 pe-0" data-coreui-toggle="dropdown" href="#"
                        ...
                        <div class="dropdown-menu dropdown-menu-end pt-0">
                            ...
                            <a class="dropdown-item"
                                href="{{ $hasAccessReviewer && request()->route('event') ? route('users.edit-profile', ['event' => request()->route('event')]) : route('users.edit-profile') }}">
        ...
        <div class="body flex-grow-1 mb-4">
            @yield('content')
        </div>

        <footer class="footer mt-auto">
            <div>
                &copy; {{ date('Y') }}. {{ env('MAIL_FROM_NAME') }}. Version 1.0 - Beta. Please contact <a
                href="mailto:submeet.cms@gmail.com">submeet.cms@gmail.com</a> if you having any difficulties.
            </div>
    ...
    <!-- <script src="{{ asset('js/main.xjs') }}"></script> -->
