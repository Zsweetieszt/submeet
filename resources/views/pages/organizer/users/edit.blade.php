...
@section('title', 'Edit Participant - ' . $user->event->event_name)
...
@section('breadcrumb')
    ...
    <li class="breadcrumb-item active"><a
            href="{{ route('dashboard.event', request()->route('event')) }}">{{ $user->event->event_name }}</a>
    </li>
    <li class="breadcrumb-item active"><a href="{{ route('organizer.users', request()->route('event')) }}">
            <span>Participants</span>
        </a>
    </li>
    <li class="breadcrumb-item"><span>Edit Participant</span>
    </li>
@endsection

<style>
    .btn-check:checked + .btn-roles {
...
@section('content')
    <div class="container-fluid px-4">
        <div class="card mb-4">
            <div class="card-header">
                <h4 class="card-title mb-0">Edit Participant - {{ $user->user->given_name . ' ' . $user->user->family_name }}</h4>
            </div>
            <div class="">
                <div class="card-body px-4">
                    <form class="needs-validation" novalidate
                        action="{{ route('organizer.users.update', [$user->event->event_code, $user->user->username]) }}"
                        method="post">
                        @csrf
                        @method('PUT')
                        <div class="row">
                                    ...
                                    <div class="form-group">
                                        @foreach ($roles as $role)
                                            <input 
                                                ...
                                                name="roles[]" 
                                                value="{{ $role->role_id }}"
                                                @if ($role->role_name === 'Paper Reviewer')
                                                    data-role="reviewer"
                                                @endif
                                                @if (in_array($role->role_id, old('roles', $user->roles))) 
                                                    checked 
                                                @endif
                                            >
                                            <label class="btn btn-secondary btn-roles" for="{{ $role->role_id }}">{{ $role->role_name }}</label>
    ...
    <script>
        ...
        //     const reviewerCheckbox = document.querySelector('input[type="checkbox"][data-role="reviewer"]');
        //     const expertiseFieldset = document.getElementById('expertiseFieldset');

        //     function toggleExpertiseFieldset() {
        //         if (reviewerCheckbox && reviewerCheckbox.checked) {
        //             expertiseFieldset.style.display = 'block';
        //         } else {
        //             expertiseFieldset.style.display = 'none';
        //         }
        //     }

        //     toggleExpertiseFieldset();

        //     if (reviewerCheckbox) {
        //         reviewerCheckbox.addEventListener('change', toggleExpertiseFieldset);
        //     }
        // });
